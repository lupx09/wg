    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className="text-sm rounded-full shadow-none focus:ring-0 bg-transparent"
        >
          {selectedModel ? (
            <div className="flex items-center space-x-1">
              ...
              {isReasoningModel(selectedModel.id) && <Lightbulb size={12} className="text-accent-blue-foreground" />}
            </div>
          ) : (
            "Select model"
          )}
                ...
                {models.map((model) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Check className={`h-4 w-4 ${value === modelId ? "opacity-100" : "opacity-0"}`} />
                    </CommandItem>
                  )
                })}
              </CommandGroup>
